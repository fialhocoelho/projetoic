...
for index, file in enumerate(fileList):
    ...
    img = np.array(Image.fromarray(img).resize((resize_to, resize_to), resample=Image.BILINEAR))
    size = int(img.shape[0] / scale)
    # resizeImg = misc.imresize(img, (size, size), interp='bilinear')
    resizeImg = np.array(Image.fromarray(img).resize((size, size), resample=Image.BILINEAR))
    # finalImg = misc.imresize(resizeImg, (img.shape[0], img.shape[0]), interp='bilinear')
    finalImg = np.array(Image.fromarray(resizeImg).resize((img.shape[0], img.shape[0]), resample=Image.BILINEAR))
